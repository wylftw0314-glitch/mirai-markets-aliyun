...
function renderSectors(){
  const query=state.sectorQuery.trim().toLowerCase(),rows=state.sectors.filter(row=>!query||`${row.name} ${row.code} ${row.leaderName} ${row.leaderCode}`.toLowerCase().includes(query)).sort((a,b)=>(Number(b[state.sectorSort])||-Infinity)-(Number(a[state.sectorSort])||-Infinity)),up=state.sectors.filter(row=>row.changePercent>0).length,down=state.sectors.filter(row=>row.changePercent<0).length,totalNet=state.sectors.reduce((sum,row)=>sum+(Number(row.mainNet)||0),0);
  $('#sector-summary').innerHTML=`<span>共 <strong>${state.sectors.length}</strong> 个板块</span><span class="flow-in">上涨 ${up}</span><span class="flow-out">下跌 ${down}</span><span>合计主力 ${totalNet>=0?'+':''}${compactCny(totalNet)}</span><span>当前显示 ${rows.length}</span>`;
  $('#sector-body').innerHTML=rows.length?rows.map(row=>`<tr class="sector-row" data-sector-code="${escapeHTML(row.code)}" tabindex="0" role="button"><td><strong>${escapeHTML(row.name)}</strong><small>${escapeHTML(row.code)} · 龙头 ${escapeHTML(row.leaderName)} ${sectorMetric(row.leaderChange,'%')}</small></td><td class="${row.changePercent>=0?'flow-in':'flow-out'}">${sectorMetric(row.changePercent,'%')}</td><td class="${row.fiveMinute>=0?'flow-in':'flow-out'}">${sectorMetric(row.fiveMinute,'%')}</td><td class="${row.relativeStrength>=0?'flow-in':'flow-out'}">${sectorMetric(row.relativeStrength,'%')}</td><td><strong>${compactCny(row.amount)}</strong><small>${row.amountSpeed==null?'等待缓存':`${row.amountSpeed>=0?'+':''}${compactCny(row.amountSpeed)}/分`}</small></td><td>${Number(row.turnover).toFixed(2)}%</td><td class="${row.mainNet>=0?'flow-in':'flow-out'}"><strong>${row.mainNet>=0?'+':''}${compactCny(row.mainNet)}</strong><small>${row.mainSpeed==null?'等待缓存':`${row.mainSpeed>=0?'+':''}${compactCny(row.mainSpeed)}/分`}</small></td><td><strong>${row.up} / ${row.down}</strong><small>广度 ${sectorMetric(row.breadth,'%')}</small></td><td>${Number(row.heat).toFixed(1)}</td></tr>`).join(''):'<tr><td colspan="9">没有匹配的板块</td></tr>';
  $$('[data-sector-code]').forEach(row=>{row.onclick=()=>openSectorDetail(row.dataset.sectorCode);row.onkeydown=event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();openSectorDetail(row.dataset.sectorCode)}}});
}
...
async function loadSectorDetail(){
  ...
  try{const data=await requestJSON('/api/china-sector-detail?code='+encodeURIComponent(sector.code)+'&interval='+state.sectorInterval+(state.sectorDate?'&date='+encodeURIComponent(state.sectorDate):''),'板块详情');renderSectorDetail(data)}catch(error){$('#sector-technical-chart').innerHTML=`<p>${escapeHTML(error.message)}</p>`;$('#sector-leader-chart').innerHTML=''}
}
...
document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'){refreshQuotes();if(!state.flowDate)loadChinaFlowSpeed()}});
