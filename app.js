...
function sectorTileSize(row,rank,total){const ratio=rank/Math.max(1,total);if(ratio<.025)return 'hero';if(ratio<.1)return 'large';if(ratio<.28)return 'medium';return 'small'}
...
function renderSectors(){
  const query=state.sectorQuery.trim().toLowerCase(),rows=state.sectors.filter(row=>!query||`${row.name} ${row.code} ${row.leaderName} ${row.leaderCode}`.toLowerCase().includes(query)).sort((a,b)=>(Number(b[state.sectorSort])||-Infinity)-(Number(a[state.sectorSort])||-Infinity)),heatOrder=[...rows].sort((a,b)=>Math.abs(Number(b.heat)||0)-Math.abs(Number(a.heat)||0)),heatRank=new Map(heatOrder.map((row,index)=>[row.code,index])),favorites=state.sectors.filter(row=>state.sectorFavorites.includes(row.code)).sort((a,b)=>state.sectorFavorites.indexOf(a.code)-state.sectorFavorites.indexOf(b.code)),regular=rows.filter(row=>!state.sectorFavorites.includes(row.code)),up=state.sectors.filter(row=>row.changePercent>0).length,down=state.sectors.filter(row=>row.changePercent<0).length,totalNet=state.sectors.reduce((sum,row)=>sum+(Number(row.mainNet)||0),0);
