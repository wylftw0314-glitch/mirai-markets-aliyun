...
async function getChinaEtfDetail(symbol,interval){
  ...
  raw.forEach(function(line){const cells=String(line).split(","),stamp=cells[0],price=Number(cells[1]);if(/^\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}/.test(stamp)&&Number.isFinite(price)&&price>0)all.push({stamp:stamp,price:price})});
  if(!all.length)throw new Error("最近交易日暂无有效分时数据");
  const tradingDate=all[all.length-1].stamp.slice(0,10),dayPoints=all.filter(function(point){return point.stamp.slice(0,10)===tradingDate}),selected=period===1?dayPoints:dayPoints.reduce(function(rows,point){const time=point.stamp.slice(11,16).split(":"),bucket=Math.floor((Number(time[0])*60+Number(time[1]))/period);if(rows.length&&rows[rows.length-1].bucket===bucket)rows[rows.length-1]={...point,bucket:bucket};else rows.push({...point,bucket:bucket});return rows},[]),today=new Intl.DateTimeFormat("en-CA",{timeZone:"Asia/Shanghai",year:"numeric",month:"2-digit",day:"2-digit"}).format(new Date());
