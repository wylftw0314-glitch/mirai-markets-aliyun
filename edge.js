...
async function getChinaEtfDetail(symbol,interval){
  ...
  const raw=chart.data&&chart.data.trends||[],all=[];
  ...
  return {symbol:etf.symbol,name:etf.name,index:etf.index,interval:period,labels:selected.map(function(point){return point.stamp.replace(" ","T")+":00+08:00"}),points:selected.map(function(point){return point.price}),opens:selected.map(function(point){return point.open}),highs:selected.map(function(point){return point.high}),lows:selected.map(function(point){return point.low}),averages:selected.map(function(point){return point.average}),volumes:selected.map(function(point){return point.volume}),amounts:selected.map(function(point){return point.amount}),sessions:selected.map(function(){return "regular"}),source:"东方财富 ETF 分时",marketDate:tradingDate,marketTimeZone:"Asia/Shanghai",marketClosed:false,fallbackDate:tradingDate!==today,previousClose:Number(quote.f60)/1000||null,price:Number(quote.f43)/1000||selected[selected.length-1].price,change:Number(quote.f169)/1000||0,changePercent:Number(quote.f170)/100||0,currency:"CNY"};
}
