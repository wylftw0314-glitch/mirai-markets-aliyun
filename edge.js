...
async function getChinaSectors(type,page){
  const kind=type==="concept"?"concept":"industry",requestedPage=Math.min(5,Math.max(1,Number(page)||1)),cacheKey="sectors_"+kind+"_"+requestedPage,cached=await edgeCacheGet(cacheKey);if(cached)return {...cached,cached:true};
  const fs=kind==="concept"?"m:90+t:3+f:!50":"m:90+t:2+f:!50",fields="f2,f3,f4,f6,f8,f11,f12,f14,f20,f62,f104,f105,f128,f140,f136,f184",headers={"User-Agent":"Mozilla/5.0","Referer":"https://quote.eastmoney.com/","Accept":"application/json"},url="https://push2.eastmoney.com/api/qt/clist/get?pn="+requestedPage+"&pz=100&po=1&np=1&fltt=2&invt=2&fid=f3&fs="+encodeURIComponent(fs)+"&fields="+fields;
  const responses=await Promise.all([fetch(url,{headers:headers}),fetch("https://push2.eastmoney.com/api/qt/stock/get?secid=1.000300&fields=f170",{headers:headers})]),json=await eastmoneyJson(responses[0],"A股板块"),benchmarkResponse=responses[1].ok?await responses[1].json():{},benchmark=Number(benchmarkResponse&&benchmarkResponse.data&&benchmarkResponse.data.f170)/100||0,raw=json&&json.data&&json.data.diff||[],previousKey="sectorprev_"+kind+"_"+requestedPage,previous=await edgeStoreGet(previousKey),now=Date.now(),elapsed=previous&&previous.savedAt?Math.max(1,(now-previous.savedAt)/60000):null,previousRows=new Map((previous&&previous.rows||[]).map(function(row){return [row.code,row]})),date=marketDate(new Date(),"Asia/Shanghai");
  ...
  const total=Number(json&&json.data&&json.data.total)||raw.length,result={type:kind,page:requestedPage,total:total,totalPages:Math.ceil(total/100),marketDate:date,updatedAt:new Date().toISOString(),benchmark:benchmark,rows:rows,source:"东方财富板块行情",cached:false};await Promise.all([edgeCachePut(cacheKey,result,120),edgeStorePut(previousKey,{savedAt:now,date:date,rows:rows.map(function(row){return {code:row.code,mainNet:row.mainNet,amount:row.amount}})})]);return result;
}
...
async function getChinaSectorLeader(market,code,date){
  if(!/^[01]$/.test(String(market))||!/^\d{6}$/.test(String(code))||!/^\d{4}-\d{2}-\d{2}$/.test(String(date)))throw new Error("龙头股参数无效");
  const cacheKey="sectorleader_"+market+"_"+code+"_"+date,cached=await edgeCacheGet(cacheKey);if(cached)return {...cached,cached:true};
  const headers={"User-Agent":"Mozilla/5.0","Referer":"https://quote.eastmoney.com/","Accept":"application/json"},url="https://push2.eastmoney.com/api/qt/stock/trends2/get?secid="+market+"."+code+"&ndays=5&iscr=0&fields1=f1,f2,f3&fields2=f51,f52,f53",response=await fetch(url,{headers:headers}),json=await eastmoneyJson(response,"龙头股分时"),raw=json&&json.data&&json.data.trends||[],points=raw.map(function(line){const cells=String(line).split(",");return {time:String(cells[0]).slice(0,16),price:Number(cells[2])}}).filter(function(point){return point.time.slice(0,10)===date&&Number.isFinite(point.price)}),result={code:String(code),points:points,source:"东方财富个股分时",cached:false};await edgeCachePut(cacheKey,result,300);return result;
}
...
async function handleRequest(request){
  ...
  if(url.pathname==="/api/china-sectors"){try{return jsonResponse(await getChinaSectors((url.searchParams.get("type")||"industry").trim(),url.searchParams.get("page")),200)}catch(error){return jsonResponse({error:error.message||"A股板块数据暂不可用"},502)}}
  ...
  if(url.pathname==="/api/china-sector-leader"){try{return jsonResponse(await getChinaSectorLeader(url.searchParams.get("market"),url.searchParams.get("code"),(url.searchParams.get("date")||"").trim()),200)}catch(error){return jsonResponse({error:error.message||"龙头股分时暂不可用"},502)}}
